...
当前限制：click 返回 action result 和 `state_diff`，但在实现 session-native target lifecycle tracking 前，不会返回 `new_tab`，也不会自动登记或切换到 click 打开的 target。
...
`dialog` 不是 v2 act kind。仓库中独立的 `bk dialog` 仍属于 v1 workspace legacy surface，不应写成 act 的 dialog 子命令。
...
### 5.3 click 触发新 tab 时的当前限制

当前 act 响应不包含 `new_tab`。即使页面通过 `target=_blank` 打开了 target，browserkit 也不会自动把它登记到 session 或切换 active tab。该返回字段和自动切换行为必须等 session-native target lifecycle tracking 实现后才能提供。
...
v2 act 当前没有 dialog kind，也没有 `blocked_by_dialog` success result contract。独立的 `bk dialog` 命令属于 v1 workspace legacy surface，不是 `bk act` 的兼容别名。
...
不要依赖 click 自动接管 `target=_blank` 打开的 tab；当前 act 不返回 `new_tab`，也不会切换到该 target。
...
**没有 v2 alias 的 legacy surface：**

- `ws *` 系列：workspace 概念移除，无法映射
- `storage *` 系列：低频，用 evaluate 替代
- `debug *` 系列：开发者工具，不适合 agent
- `dialog *` 系列：仍是 v1 workspace legacy surface；v2 act 没有 dialog kind
...
- v2 核心命令使用 snapshot/act/navigate 等新 surface。
- click/type 等旧 action 命令已经移除，不提供 executable aliases。
- ws/tab/browser/daemon/storage/dialog/debug 等显式标记为 Legacy 的 v1 命令仍独立存在，计划在 Phase 3 清理。

## 12. 实施计划

> **历史规划记录（非当前 CLI contract）**：本节保留早期阶段目标用于追溯。未完成条目不能作为当前可执行命令或响应行为；当前 action contract 以前文和 `bk act --help` 为准。
