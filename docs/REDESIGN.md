...
> Only-agent 浏览器自动化工具 -- 为 LLM agent 而生

## 1. 设计哲学

### 1.1 Only-agent 意味着什么
...
|------|---------------|-------------|---------------|
| 接入方式 | MCP Server（需 MCP 客户端） | Python SDK（需 LangChain） | CLI（任何 agent 通过 Bash tool） |
...
1. **CLI-native**：不需要 MCP 协议栈，任何能调 Bash 的 agent 直接用
2. **接管真实浏览器**：操作用户已登录的 Chrome，不用处理登录/cookie 导入
3. **daemon 持久化**：浏览器连接跨 agent 会话保持，不用每次重连
...
- 在同一个 BrowserContext 中打开的新 tab（包括 click 触发的 `target=_blank`）天然归属本 session，由 Chrome 内核保证，无竞态问题
...
- BrowserContext 内打开的所有 tab（包括 click 触发的新 tab）天然归属本 session
- Chrome 内核保证新 tab 在创建它的 BrowserContext 中打开
- 无竞态：即使多个 agent 同时 click 触发新 tab，每个新 tab 自动归属正确的 session
...
| `act click` 触发新 tab（`target=_blank`） | 新 tab 成为 active tab |
...
**Phase 2 扩展**：如果将来需要接管用户 tab，通过 `bk attach --target <tid>` 显式授权，不会默认开放。
...
**Headless 独立 Chrome 进程**：Phase 2 通过 `--headless` flag 支持，启动独立的无头 Chrome 进程。不影响当前 session 设计——headless Chrome 只是另一个可以创建 session 的浏览器实例。
...
| `type --index N "text"` | `act type --ref 42 "text"` | 合并进 act，默认 clear |
| `fill --set ...` | `act fill --set ...` | 合并进 act |
| `select --index N "val"` | `act select --ref 42 "val"` | 合并进 act |
| `scroll down` | `act scroll down` | 合并进 act |
| `hover --index N` | `act hover --ref 42` | 合并进 act |
| `keys Enter` | `act press Enter` | 合并进 act，重命名为 press |
| `drag ...` | `act drag ...` | 合并进 act |
| `upload ...` | `act upload ...` | 合并进 act |
...
| `find "selector"` | 删除 | agent 用 snapshot + evaluate 替代 |
| `search "text"` | 删除 | snapshot 的 page_text 已含页面文本 |
| `html` | 删除 | evaluate 替代 |
| `url` | 删除 | snapshot 返回中包含 |
| `title` | 删除 | snapshot 返回中包含 |
| `console` | 删除 | evaluate 替代 |
| `options --index N` | 删除 | snapshot 对 select 元素自动展示选项 |
| `pdf` | 删除 | agent 场景不需要 |
| `fetch` | 删除 | navigate + evaluate 替代 |
| `ws *` | 删除 | session 替代，workspace 不暴露给 agent |
| `storage *` | 删除 | 低频，用 evaluate 操作 cookie/localStorage |
| `dialog *` | 删除（自动处理） | dialog 默认 auto-dismiss，act 返回中报告 |
| `debug *` | 删除 | evaluate 作为逃生舱 |
...
```
bk snapshot [--session <name>] [--target <targetId>] [--full] [--timeout <ms>]
```
...
```
bk act <kind> [args...] [--session <name>] [--target <targetId>] [--timeout <ms>] [--no-state-diff]
```
...
特殊行为：如果 click 触发新 tab（target=_blank），新 tab 自动成为 session active tab（不自动 focus，需要时显式加 `--focus`）。返回中包含 `new_tab` 字段。

**kind = type**

```
bk act type --ref <N> "text to type" [--session <name>]
```
...
```
bk act press <keys...> [--session <name>]
```
...
```
bk act scroll <direction> [--amount <px>] [--session <name>]
bk act scroll --ref <N>
```
...
```
bk act select --ref <N> "option value or text" [--session <name>]
```
...
```
bk act drag --from <N> --to <N> [--session <name>]
```
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `--ref` | i64 | file input 元素的 ref |
| `file_path` | string[] | 绝对文件路径列表 |

**kind = dialog**

```
bk act dialog accept [--text "prompt response"] [--session <name>]
bk act dialog dismiss [--session <name>]
```

| 参数 | 类型 | 说明 |
|------|------|------|
| action | string | accept / dismiss |
| `--text` | string | prompt 类型对话框的输入文本 |
...
```
bk evaluate "<expression>" [--session <name>] [--target <targetId>] [--timeout <ms>]
bk evaluate --file <path> [--session <name>] [--target <targetId>]
```
...
`new_tab` 字段出现时，表示 click 触发了新 tab 打开。新 tab 自动成为 session active tab（不自动 focus）。后续不带 `--target` 的命令将操作新 tab。
...
当 act 触发了 JavaScript 对话框（alert/confirm/prompt）且策略为 manual 时：
...
agent 看到 `blocked_by_dialog` 后，需要决定 accept 或 dismiss：

```bash
bk act dialog accept --target E3B2A1F09C7D4A68
```

注意：v2 默认 dialog 策略为 auto-dismiss（适合大多数场景）。只有明确需要处理 dialog 时才设置为 manual。
...
| `DIALOG_BLOCKING` | JavaScript 对话框阻塞了操作 | true | handle the dialog first: bk act dialog accept/dismiss |
...
10. **`DIALOG_BLOCKING`**：agent 应该先处理对话框再继续操作。
...
bk act click --ref 101 --session a             # 触发新 tab-2，active 切换到新 tab
# act 返回 new_tab: "TAO222"
bk snapshot --session a                         # 自动操作新 active tab（tab-2）
...
| 新 tab 归属 | 推断，有竞态 | isolated: BrowserContext 天然归属；default: 触发者继承 |
...
- `storage *` 系列：低频，用 evaluate 替代
- `debug *` 系列：开发者工具，不适合 agent
- `dialog *` 系列：被 auto-dismiss + `act dialog` 替代
...
- v2.0.0：实现核心命令（snapshot/act/navigate），旧命令带 deprecated warning
- v2.1.0：移除所有 deprecated 别名
- v1.x：不再维护，最终版本冻结
...
| Tab 创建检查 | open/click 新 tab 时检查 max_tabs_per_session 限制，超出返回 TAB_LIMIT_EXCEEDED |
...
| 模块 | 变化 |
|------|------|
| `handler/act.rs` | 完善所有 kind：fill/select/hover/focus/drag/upload/dialog/press |
| `handler/dialog.rs` | 改造：默认策略改为 auto-dismiss；manual 模式通过 act dialog 操作 |
| Session 超时 | 72h 无操作自动 close session + 销毁 BrowserContext |
| Active tab 追踪 | click 触发新 tab 时自动切换 active tab |
...
| 模块 | 变化 |
|------|------|
| 网络响应观察 | 新增 `bk network watch --pattern "<url-glob>" [--session <name>]`，订阅匹配 URL pattern 的网络响应，返回响应 body（JSON 格式）。典型用途：拦截 XHR/fetch 响应，直接获取结构化数据而不解析渲染 DOM |
| 文件下载处理 | click 下载链接后，act 返回中包含 `download: {filename, path, status}` 字段；新增 `bk download wait [--timeout <ms>]` 等待下载完成。通过 CDP `Browser.downloadWillBegin` 和 `Browser.downloadProgress` 事件实现 |
| evaluate 追加写入 | 新增 `evaluate --append-to <file>` 参数，evaluate 结果追加写入本地文件。解决批量抓取时 agent context 溢出问题。典型用途：每页提取数据后追加写入 results.jsonl |

**Phase 2 交付标准：**

- 所有 act kind 都能正常工作
- dialog 默认 auto-dismiss，act 返回中正确报告 blocked_by_dialog
- snapshot --full 模式提供完整信息
- page_text 有 [PAGE_CONTENT_START/END] wrapping
- session 超时自动清理
- click 触发新 tab 时 active tab 正确切换
...
| 模块 | 变化 |
|------|------|
| `src/main.rs` | 移除所有 deprecated 别名；删除旧命令定义；清理 Command enum |
