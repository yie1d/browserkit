...
| `network watch` | 有界观察 XHR/fetch 响应 | 低 |
...
命令先订阅 Network events 再 enable 域，只收集 URL 包含 pattern 的 XHR/fetch。达到 count 或 timeout 后返回一个 JSON response，包含 `responses`、`response_count`、`stop_reason` 和 `timed_out`；它不是 streaming route，也不会恢复已删除的 debug monitor/har/events。
...
| 模块 | 变化 |
|------|------|
| 网络响应观察 | 当前合同为 `bk network watch --pattern <url-substring> [--count N]`；单次请求有 count/timeout 边界，只观察 XHR/fetch，不恢复旧 streaming route |
