...
| 功能 | CLI 命令 | 协议命令 | 说明 |
|------|---------|---------|------|
| 视口截图 | `bk screenshot` / `bk shot` | `screenshot` | PNG 格式截图 |
| 全页截图 | `bk screenshot --full-page` | `screenshot` | 完整可滚动页面截图 |
| 元素截图 | `bk shot --selector "#id"` | `screenshot` | CSS 选择器定位元素截图 |
...
| 同步执行 | `bk eval --sync "expr"` | `evaluate` (`await_promise=false`) | deprecated alias |
