...
browserkit 是一个基于 [cdpkit](https://crates.io/crates/cdpkit)（Rust 类型化 CDP 客户端）构建的浏览器自动化工具包。提供 CLI 工具 `bk` 和后台守护进程（daemon），通过 Chrome DevTools Protocol (CDP) 控制 Chrome/Chromium 浏览器。
...
| 许可证 | MIT |
| 二进制名 | `bk` |
| 核心依赖 | cdpkit 0.2.2, tokio 1, clap 4 |
| 源码文件 | 32 个 (.rs) |
| 源码行数 | ~6,800 行 |
| 测试文件 | 13 个 (.rs) |
| 测试行数 | ~3,700 行 |
...
| 功能 | CLI 命令 | 协议命令 | 说明 |
|------|---------|---------|------|
| 创建工作区 | `bk ws new` / `bk new` | `ws.new` | 创建隔离的浏览器上下文 |
| 列出工作区 | `bk ws list` / `bk ls` | `ws.list` | 列出所有工作区 |
| 工作区详情 | `bk ws info [wid]` | `ws.info` | 查看工作区详细信息 |
| 关闭工作区 | `bk ws close <wid>` / `bk rm` | `ws.close` | 关闭并清理工作区 |
...
| 功能 | CLI 命令 | 协议命令 | 说明 |
|------|---------|---------|------|
| 导航到 URL | `bk goto <url>` / `bk nav goto` | `nav.goto` | 带重试的页面导航 |
| 刷新页面 | `bk reload` / `bk nav reload` | `nav.reload` | 刷新当前页面 |
| 后退 | `bk nav back` | `nav.back` | 浏览器历史后退 |
| 前进 | `bk nav forward` | `nav.forward` | 浏览器历史前进 |
| 获取 URL | `bk nav url` | `nav.url` | 获取当前页面 URL |
| 获取标题 | `bk nav title` | `nav.title` | 获取当前页面标题 |
| 等待加载 | `bk nav wait` | `nav.wait` | 等待页面加载完成 |
...
| 功能 | CLI 命令 | 协议命令 | 说明 |
|------|---------|---------|------|
| 交互元素列表 | `bk page state` | `page.state` | 发现页面上所有可交互元素 |
| 带截图状态 | `bk page state --screenshot` | `page.state` | 同时返回视口截图 |
...
| 功能 | CLI 命令 | 协议命令 | 说明 |
|------|---------|---------|------|
| 视口截图 | `bk shot` | `page.screenshot` | PNG 格式截图 |
| 全页截图 | `bk shot --full-page` | `page.screenshot` | 完整可滚动页面截图 |
| 元素截图 | `bk shot --selector "#id"` | `page.screenshot` | CSS 选择器定位元素截图 |
| 保存截图 | `bk shot -o file.png` | `page.screenshot` | 保存到文件 |
| 一键截图 | `bk shot <url> -o file.png` | - | 一次性：创建→导航→截图→关闭 |
...
| 功能 | CLI 命令 | 协议命令 | 说明 |
|------|---------|---------|------|
| 执行表达式 | `bk eval "expr"` | `js.eval` (await=true) | 异步执行 JS |
| 同步执行 | `bk js eval "expr"` | `js.eval` (await=false) | 同步执行 JS |
| 执行文件 | `bk js file script.js` | `js.eval` | 读取并执行 JS 文件（≤5MB） |
