...
- `daemon/` — 生命周期 `mod.rs`、状态 `state.rs`(`Arc<DaemonState>` + DashMap)、TCP server `server.rs`、防抖持久化 `persist.rs`、协议 `protocol.rs`、`handler/`(每命令组一文件:workspace/tab/nav/page/action/js/storage/browser/network/debug/daemon/common)。
- `page/` — navigation / interaction / capture / state。
- `workspace/mod.rs` — Workspace + Tab 类型（内部/legacy runtime 状态;对外优先暴露 session）。

## 关键设计（改之前先尊重）

- **Session** = agent-facing 隔离模型。默认 session 复用用户 Chrome 登录态;命名 session 用 BrowserContext 隔离 cookie/storage/tab。
- **Workspace** = 内部/legacy runtime 状态,仍服务旧命令与部分持久化/handler;不要在新的对外 API 文档里把 workspace 当主概念推广。
- **持久化**:`~/.bk/state.json` 为统一状态文件,旧 `browsers.json`/`workspaces.json`/`default_ws` 仅作为迁移来源;`daemon.port` 记录 daemon 端口。写入原子(tmp+rename)且防抖(500ms)。别在请求处理里同步阻塞写盘。
...
- `Cargo.toml` 当前锁 `cdpkit = "0.3.0"`(crates.io 已发布版)。底层 API 变更不会自动传导。
- 0.3.0 使用 unified Sender trait API:浏览器级命令 `cmd.send(&cdp)`;页面级命令 `let session = cdp.session(session_id); cmd.send(&session)`;`tokio::spawn` 场景用 `cdp.owned_session(id)` 得到 `OwnedSession`(Send+'static)。
