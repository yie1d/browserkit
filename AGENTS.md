...
- **持久化**:`~/.bk/state.json` 为 schema v3 session-only 状态文件。v2 state 会先备份为 `state.v2.backup*.json` 再迁移到 v3;运行时不再写 workspace 字段。`daemon.port` 记录 daemon 端口。写入原子(tmp+rename)且防抖(500ms)。别在请求处理里同步阻塞写盘。
